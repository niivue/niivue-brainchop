...
async function getAllSlices2D(allSlices, slice_height, slice_width) {
  ...
  for (let sliceIdx = 0; sliceIdx < allSlices.length; sliceIdx++) {
    allSlices_2D.push(
      tf.tensor(allSlices[sliceIdx], [slice_height, slice_width]),
    );
  }
  return allSlices_2D;
}
...
async function quantileNormalizeVolumeData(
  tensor,
  lowerQuantile = 0.05,
  upperQuantile = 0.95,
) {
  // Call calculateQuantiles and wait for the result
  const { qmin, qmax } = await calculateQuantiles(
    tensor,
    lowerQuantile,
    upperQuantile,
  );
...
async function minMaxNormalizeVolumeData(volumeData) {
  ...
  const normalizedSlices_3d = await volumeData
    .sub(volumeData_Min)
    .div(volumeData_Max.sub(volumeData_Min));
...
async function draw3dObjBoundingVolume(
  ...
  for (
    let sliceTensorIdx = 0;
    sliceTensorIdx < unstackOutVolumeTensor.length;
    sliceTensorIdx++
  ) {
    allOutputSlices3DCC[sliceTensorIdx] = Array.from(
      unstackOutVolumeTensor[sliceTensorIdx].dataSync(),
    );
  }
  ...
  const allOutputSlices3DCC1DimArray = new Array(
    allOutputSlices3DCC[0].length * allOutputSlices3DCC.length,
  );
  let index = 0;
  ...
  let brainMaskTensor1d = await binarizeVolumeDataTensor(
    tf.tensor1d(allOutputSlices3DCC1DimArray),
  );
...
async function addZeroPaddingTo3dTensor(
  tensor3d,
  rowPadArr = [1, 1],
  colPadArr = [1, 1],
  depthPadArr = [1, 1],
...
async function removeZeroPaddingFrom3dTensor(
  tensor3d,
  rowPad = 1,
  colPad = 1,
  depthPad = 1,
  ...
  return tensor3d.slice(
    [rowPad, colPad, depthPad],
    [h - 2 * rowPad, w - 2 * colPad, d - 2 * depthPad],
  );
}

async function resizeWithZeroPadding(
  croppedTensor3d,
  newDepth,
  newHeight,
  newWidth,
  ...
  const row_pad_after =
    newHeight - row_max - 1 > 0 ? newHeight - row_max - 1 : 0;
  const col_pad_after = newWidth - col_max - 1 > 0 ? newWidth - col_max - 1 : 0;
  const depth_pad_after =
    newDepth - depth_max - 1 > 0 ? newDepth - depth_max - 1 : 0;
...
async function generateBrainMask(
  ...
  if (opts.isBrainCropMaskBased) {
    //  Mask-based
    const brainMaskTensor1d = await binarizeVolumeDataTensor(
      tf.tensor1d(allOutputSlices3DCC1DimArray),
    );
    brainOut = Array.from(brainMaskTensor1d.dataSync());
  } else {
    //  Brain tissue
    let allSlices = await getAllSlicesData1D(
      num_of_slices,
      niftiHeader,
      niftiImage,
    );
    ...
    for (let sliceIdx = 0; sliceIdx < allOutputSlices3DCC.length; sliceIdx++) {
      for (
        let pixelIdx = 0;
        pixelIdx < slice_height * slice_width;
        pixelIdx++
      ) {
...
function processTensorInChunks(inputTensor, filterWeights, chunkSize) {
  ...
  for (let i = 0; i < numSlices; i++) {
    ...
    const filterSlice = tf.tidy(() => {
      ...
      return filterWeights.slice(
        [0, 0, 0, startChannel, 0],
        [-1, -1, -1, channels, -1],
      );
    });
...
class SequentialConvLayer {
  ...
  async apply(inputTensor) {
    ...
    const outputShape = self.isChannelLast
    ...
    while (true) {
      ...
      const result = await tf.tidy(() => {
        const filterWeights = weights.slice(
          [0, 0, 0, 0, chIdx],
          [-1, -1, -1, -1, 1],
        );
      ...
      console.log(
        `Megabytes In Use: ${(memoryInfo.numBytes / 1048576).toFixed(3)} MB`,
      );
      ...
      if (chIdx === self.outChannels - 1) {
        ...
        console.log(
          `Execution time for output layer: ${executionTime} milliseconds`,
        );
...
async function generateOutputSlicesV2(
  ...
  if (opts.isPostProcessEnable) {
    ...
    const [labelCount, labeledImage] = BWInstance.bwlabel(
      img,
      dim,
      conn,
      binarize,
  ...
  if (modelEntry.preModelId) {
    ...
    try {
      ...
      if (isPreModelChannelLast) {
        ...
        if (
          isNaN(preModelBatchInputShape[4]) ||
        ...
        if (
          isNaN(preModelBatchInputShape[1]) ||
...
async function runInferenceWW(
  ...
  const allSlices_2D = await getAllSlices2D(
    allSlices,
    slice_height,
    slice_width,
  );
